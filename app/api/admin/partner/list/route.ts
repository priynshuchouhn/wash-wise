...
export async function GET(request: NextRequest) {
    try {
        ...
        const lstCustomer = await User.find({role:'Delivery Partner'}).select('name email _id registrationDate');
       return NextResponse.json({message: 'Delivery Partner List Fetched Successfully', data:lstCustomer}, {status: 200});
    } catch (error) {
